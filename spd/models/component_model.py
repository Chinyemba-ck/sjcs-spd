...
class ComponentModel(LoadableModule):
    ...
    def from_run_info(cls, run_info: RunInfo[Config]) -> "ComponentModel":
        ...
        # Handle backwards compatibility for old checkpoint format
        if any(k.startswith("model.") for k in comp_model_weights.keys()):
            print("Detected old checkpoint format, converting to new format...")
            comp_model_weights = cls._convert_old_state_dict(comp_model_weights)
